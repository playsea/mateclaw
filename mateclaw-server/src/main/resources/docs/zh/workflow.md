...
- 不是 30+ 节点的 Dify-style 编排——DigitalClaw 工作流刻意保持极简：**线性 step 数组 + 一个 mode 字段表达控制流**
...
- `actingAgentId`：sequential / 3 个 DigitalClaw mode → 该 step 的 agent；其它 mode → 发布者作为 fallback
